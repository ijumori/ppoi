...
| FR-09 | 格言が「創作（っぽい格言）」であると明示される | High | ✅ v1.1 | 誤認防止（UI+共有画像+オンボード） |
| FR-10 | ユーザーは格言本文の字体をA/Bで切替できる | Med | 一部 | serif / default |
| FR-11 | ユーザーは当日の格言をお気に入り保存できる | Med | ✅ v1.1 | ローカルのみ |
| FR-12 | ユーザーは連続閲覧日数（ストリーク）を確認できる | Med | ✅ v1.1 | 再訪動機 |
| FR-13 | ユーザーはホーム画面ウィジェットで今日の格言を見られる | Med | ✅ v1.2 | WidgetKit + App Group |
| FR-14 | ユーザーは買い切り課金で広告除去+全機能解放できる | Med | ✅ v2.0 | StoreKit 2 非消費型 |
| FR-15 | アプリ下部にバナー広告が表示される | High | 実装済 | 課金時は非表示（v2.0） |
| FR-16 | 𝕏共有後にインタースティシャル広告が表示される | High | 実装済 | 課金時は非表示（v2.0） |
...
| # | タスク | 関連FR | 規模 | 状態 |
|---|--------|--------|------|------|
| W1 | 共有画像とUIに「っぽい格言（創作）」明示を追加 | FR-09 | S | ✅ v1.1 |
| W2 | お気に入り保存（ローカル）モデル+UI | FR-11 | M | ✅ v1.1 |
| W3 | ストリーク計測ロジック+ホーム表示 | FR-12 | M | ✅ v1.1 |
| W4 | WidgetKit ウィジェット（今日の格言） | FR-13 | L | ✅ v1.2 |
| W5 | StoreKit 2 買い切り（広告除去+全機能） | FR-14 | L | ✅ v2.0 |
| W6 | 課金フラグによる広告非表示制御 | FR-14,15,16 | S | ✅ v2.0 |
| W7 | ペイウォール画面 | FR-14 | M | ✅ v2.0 |
| W8 | 設定画面に課金/復元導線追加 | FR-14 | S | ✅ v2.0 |

### リリース分割
- **v1.1**: ✅ W1（創作明示）+ W2（お気に入り）+ W3（ストリーク）— 実装済。
- **v1.2**: ✅ W4（ウィジェット）— 実装済。
- **v2.0**: ✅ W5〜W8（買い切り課金）— 実装済（承認済 §Phase01-4）。

> WBS 全タスク実装完了。残るは Mac での `xcodegen generate` → ビルド/実機・Sandbox 確認、および ASC 側の IAP・App Groups 設定。
