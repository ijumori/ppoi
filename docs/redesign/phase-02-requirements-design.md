...
| W6 | 課金フラグによる広告非表示制御 | FR-14,15,16 | S | ⬜ v2.0（W5依存） |
| W7 | ペイウォール画面 | FR-14 | M | ⬜ v2.0（W5依存） |
| W8 | 設定画面に課金/復元導線追加 | FR-14 | S | ⬜ v2.0（W5依存） |

### リリース分割
- **v1.1**: ✅ W1（創作明示）+ W2（お気に入り）+ W3（ストリーク）— 実装済。
- **v1.2**: ✅ W4（ウィジェット）— 実装済。
- **v2.0**: ⬜ W5〜W8（買い切り課金）— 未着手（承認済 §Phase01-4）。

> 実装は完了済だが、Mac での `xcodegen generate` → ビルド/実機確認は未実施。
